#define BOOST_TEST_MODULE Test_Sketcher

#include "../test_common.h"
#include "schrodinger/sketcher/molviewer/atom_display_settings.h"
#include "schrodinger/sketcher/molviewer/bond_display_settings.h"
#include "schrodinger/sketcher/molviewer/fonts.h"
#include "schrodinger/sketcher/molviewer/monomer_utils.h"
#include "schrodinger/sketcher/molviewer/scene_utils.h"
#include "schrodinger/sketcher/rdkit/mol_update.h"
#include "schrodinger/rdkit_extensions/convert.h"

BOOST_GLOBAL_FIXTURE(QApplicationRequiredFixture);

using schrodinger::rdkit_extensions::Format;

namespace schrodinger
{
namespace sketcher
{

/**
 * Make sure that create_graphics_items_for_mol() creates the expected number of
 * graphics items for monomeric models, including returns two separate graphics
 * items for bond_items with a secondary connections.  Also ensure that
 * get_model_objects_for_graphics_items() can return the corresponding model
 * objects.
 */
BOOST_AUTO_TEST_CASE(test_create_graphics_items_for_mol_monomeric)
{
    AtomDisplaySettings atom_display_settings;
    BondDisplaySettings bond_display_settings;
    Fonts fonts;
    {
        // two neighboring cysteines, but no disulfidea
        auto mol = rdkit_extensions::to_rdkit("PEPTIDE1{C.C}$$$$V2.0");
        prepare_mol(*mol);
        for (auto* atom : mol->atoms()) {
            set_atom_monomeric(atom);
        }
        auto [all_items, atom_items, bond_items, secondary_connection_items,
              sgroup_items] =
            create_graphics_items_for_mol(
                mol.get(), fonts, atom_display_settings, bond_display_settings);
        BOOST_TEST(all_items.size() == 3);
        BOOST_TEST(atom_items.size() == 2);
        BOOST_TEST(bond_items.size() == 1);
        BOOST_TEST(secondary_connection_items.empty());
        BOOST_TEST(sgroup_items.empty());

        std::unordered_set<QGraphicsItem*> all_items_set;
        std::copy(all_items.begin(), all_items.end(),
                  std::inserter(all_items_set, all_items_set.end()));
        auto [atoms, bonds, secondary_connections, sgroups,
              non_molecular_objects] =
            get_model_objects_for_graphics_items(all_items_set);
        BOOST_TEST(atoms.size() == 2);
        BOOST_TEST(bonds.size() == 1);
        BOOST_TEST(secondary_connections.empty());
        BOOST_TEST(sgroups.empty());
        BOOST_TEST(non_molecular_objects.empty());
    }
    {
        // two neighboring cysteines with a disulfide
        auto mol = rdkit_extensions::to_rdkit(
            "PEPTIDE1{C.C}$PEPTIDE1,PEPTIDE1,1:R3-2:R3$$$V2.0");
        prepare_mol(*mol);
        for (auto* atom : mol->atoms()) {
            set_atom_monomeric(atom);
        }
        auto [all_items, atom_items, bond_items, secondary_connection_items,
              sgroup_items] =
            create_graphics_items_for_mol(
                mol.get(), fonts, atom_display_settings, bond_display_settings);
        BOOST_TEST(all_items.size() == 4);
        BOOST_TEST(atom_items.size() == 2);
        BOOST_TEST(bond_items.size() == 1);
        BOOST_TEST(secondary_connection_items.size() == 1);
        BOOST_TEST(sgroup_items.empty());

        std::unordered_set<QGraphicsItem*> all_items_set;
        std::copy(all_items.begin(), all_items.end(),
                  std::inserter(all_items_set, all_items_set.end()));
        auto [atoms, bonds, secondary_connections, sgroups,
              non_molecular_objects] =
            get_model_objects_for_graphics_items(all_items_set);
        BOOST_TEST(atoms.size() == 2);
        BOOST_TEST(bonds.size() == 1);
        BOOST_TEST(secondary_connections.size() == 1);
        BOOST_TEST(sgroups.empty());
        BOOST_TEST(non_molecular_objects.empty());
    }
    {
        // a disulfide, but between two non-neighboring cysteines
        auto mol = rdkit_extensions::to_rdkit(
            "PEPTIDE1{C.A.C}$PEPTIDE1,PEPTIDE1,1:R3-3:R3$$$V2.0");
        prepare_mol(*mol);
        for (auto* atom : mol->atoms()) {
            set_atom_monomeric(atom);
        }
        auto [all_items, atom_items, bond_items, secondary_connection_items,
              sgroup_items] =
            create_graphics_items_for_mol(
                mol.get(), fonts, atom_display_settings, bond_display_settings);
        BOOST_TEST(all_items.size() == 6);
        BOOST_TEST(atom_items.size() == 3);
        BOOST_TEST(bond_items.size() == 3);
        BOOST_TEST(secondary_connection_items.size() == 0);
        BOOST_TEST(sgroup_items.empty());

        std::unordered_set<QGraphicsItem*> all_items_set;
        std::copy(all_items.begin(), all_items.end(),
                  std::inserter(all_items_set, all_items_set.end()));
        auto [atoms, bonds, secondary_connections, sgroups,
              non_molecular_objects] =
            get_model_objects_for_graphics_items(all_items_set);
        BOOST_TEST(atoms.size() == 3);
        BOOST_TEST(bonds.size() == 3);
        BOOST_TEST(secondary_connections.empty());
        BOOST_TEST(sgroups.empty());
        BOOST_TEST(non_molecular_objects.empty());
    }
}

} // namespace sketcher
} // namespace schrodinger
