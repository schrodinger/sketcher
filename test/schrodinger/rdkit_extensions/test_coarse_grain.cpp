...
{
    ::RDKit::RWMol cg_mol;
    auto monomer_idx1 = addMonomer(cg_mol, "A", 1, "PEPTIDE1");
    auto monomer_idx2 = addMonomer(cg_mol, "G");
    auto monomer_idx3 = addMonomer(cg_mol, "C");
    addConnection(cg_mol, monomer_idx1, monomer_idx2);
    addConnection(cg_mol, monomer_idx2, monomer_idx3);
    assignChains(cg_mol);
...
BOOST_AUTO_TEST_CASE(TestBranchesCoarseGrain)
{
    ::RDKit::RWMol cg_mol;
    auto monomer_idx1 = addMonomer(cg_mol, "A", 1, "PEPTIDE1");
    auto monomer_idx2 = addMonomer(cg_mol, "C");
    auto monomer_branch = addMonomer(cg_mol, "A");
    auto monomer_idx3 = addMonomer(cg_mol, "C");
    auto monomer_idx4 = addMonomer(cg_mol, "T");

    addConnection(cg_mol, monomer_idx1, monomer_idx2);
    addConnection(cg_mol, monomer_idx2, monomer_idx3);
    addConnection(cg_mol, monomer_idx3, monomer_idx4);
    addConnection(cg_mol, monomer_idx2, monomer_branch,
                  ConnectionType::SIDECHAIN);
    assignChains(cg_mol);
    BOOST_CHECK_EQUAL(to_string(cg_mol, Format::HELM),
                      "PEPTIDE1{A.C(A)C.T}$$$$V2.0");

    auto monomer_branch2 = addMonomer(cg_mol, "A");
    addConnection(cg_mol, monomer_idx3, monomer_branch2,
                  ConnectionType::SIDECHAIN);
    assignChains(cg_mol);
    BOOST_CHECK_EQUAL(to_string(cg_mol, Format::HELM),
...
BOOST_AUTO_TEST_CASE(TestMultipleChainsCoarseGrainMol)
{
    ...
    auto monomer_idx4 = addMonomer(cg_mol, "T", 1, "PEPTIDE2");
    auto monomer_idx5 = addMonomer(cg_mol, "C");
    auto monomer_idx6 = addMonomer(cg_mol, "A");
    addConnection(cg_mol, monomer_idx4, monomer_idx5);
    addConnection(cg_mol, monomer_idx5, monomer_idx6);
...
{
    {
        ...
        auto cg_mol = atomisticToCg(*mol, use_residue_info);
        BOOST_CHECK_EQUAL(to_string(*cg_mol, Format::HELM),
    ...
    // memtest is ran
    {
        auto cg_mol =
            to_rdkit("PEPTIDE1{C.A.A.A.C}$PEPTIDE1,PEPTIDE1,1:R3-5:R3$$$V2.0",
                     Format::HELM);
        auto atomistic_mol = cgToAtomistic(*cg_mol);
...
{
    ::RDKit::RWMol cg_mol;
    addMonomer(cg_mol, "D", 3, "PEPTIDE1");
    addMonomer(cg_mol, "C", 1, "PEPTIDE1");
    addMonomer(cg_mol, "B", 4, "PEPTIDE1");
    addMonomer(cg_mol, "A", 2, "PEPTIDE1");
    addConnection(cg_mol, 3, 2);
    addConnection(cg_mol, 2, 1);
    addConnection(cg_mol, 1, 0);
    assignChains(cg_mol);
