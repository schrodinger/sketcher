...
def test_importing(page: Page, format_name: str):
    ...
    page.evaluate("() => Module.sketcher_clear()")
    page.evaluate("(text) => Module.sketcher_import_text(text)", input_text)
...
def test_exporting_image(page: Page, image_format: str):
    ...
    if image_format == "SVG":
        ...
        expected_image = snapshot_path.read_bytes()
        assert image_buffer == expected_image
