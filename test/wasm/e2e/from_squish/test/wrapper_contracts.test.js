...
test.describe('Sketcher wrapper contracts', () => {
  ...
  test('Export menu selects SDF through the visible File Export dialog', async ({ page }) => {
    ...
    // This contract test deliberately checks only the shared UI path. The
    // complete export suite checks the bridge-observed generated bytes.
    await sk.export_menu({ filename: 'contract_export', format: 'SDF', observeDownload: false });
  });
