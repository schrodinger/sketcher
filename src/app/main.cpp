...
#ifdef SKETCHER_ENABLE_PLAYWRIGHT_TEST_BRIDGE
    // Generic geometry interface under evaluation. Keep legacy bindings below
    // until converted wrappers have passed their parity checks against it.
    emscripten::function(
        "_sketcher_get_rect", +[](const std::string& selector) {
            return schrodinger::sketcher::playwright_test_bridge::get_rect(
                get_sketcher_instance(), selector);
        });
    emscripten::function(
        "_sketcher_get_popup_owner", +[](const std::string& name) {
            return schrodinger::sketcher::playwright_test_bridge::
                get_popup_owner(get_sketcher_instance(), name);
        });
    emscripten::function(
        "_sketcher_activate_action", +[](const std::string& name_or_text) {
            schrodinger::sketcher::playwright_test_bridge::activate_action(
