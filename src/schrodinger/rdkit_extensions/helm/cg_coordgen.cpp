...
constexpr uint MONOMERS_PER_SNAKE = 10;
...
static std::vector<int> find_largest_ring(const RDKit::ROMol& polymer)
{
    ...
    for (auto cycle : polymer.getRingInfo()->atomRings()) {
        ...
        std::vector<uint> connection_points{};
...
find_hairpin_turn(const RDKit::ROMol& polymer)
{
    ...
    for (uint monomer_idx : hairpin_turn_idxs) {
        auto monomer = polymer.getAtomWithIdx(monomer_idx);
...
{
    std::map<uint, RDKit::ROMOL_SPTR> polymer_for_monomer_idx;
    for (auto polymer : polymers) {
        for (auto monomer : polymer->atoms()) {
            auto monomer_idx = monomer->getProp<uint>(ORIGINAL_INDEX);
    ...
    for (auto polymer : polymers) {
        ...
        for (polymer_queue.push(polymer); !polymer_queue.empty();
            ...
            for (auto monomer : polymer->atoms()) {
                ...
                auto neighbor_monomer = monomer->getProp<uint>(BOND_TO);
...
typedef std::vector<std::pair<uint, uint>> BOND_IDX_VEC;
static BOND_IDX_VEC get_bonds_between_polymers(const RDKit::ROMol& from,
                                               const RDKit::ROMol& to)
{
    std::map<uint, RDKit::Atom*> end_monomers;
    for (auto monomer : to.atoms()) {
        end_monomers[monomer->getProp<uint>(ORIGINAL_INDEX)] = monomer;
    }
    ...
    for (auto monomer : from.atoms()) {
        ...
        auto bond_end_idx = monomer->getProp<uint>(BOND_TO);
    ...
    for (auto polymer : polymers) {
        for (auto monomer : polymer->atoms()) {
            conformer->setAtomPos(
                monomer->getProp<uint>(ORIGINAL_INDEX),
