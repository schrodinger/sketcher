...
/**
 * Lay out a chain from a ring along the cardinal direction closest to the
 * attachment geometry generated by RDKit. Only the ring coordinates are used
 * as fixed anchors; the first chain monomer is placed one standard bond length
 * from the ring instead of retaining its potentially diagonal RDKit position.
 */
static void
lay_out_chain_from_ring(RDKit::ROMol& polymer, const RDKit::Atom* ring_monomer,
                        const RDKit::Atom* first_chain_monomer,
                        std::unordered_set<int>& placed_monomers_idcs)
{
    auto& conformer = polymer.getConformer();
    const auto ring_pos = conformer.getAtomPos(ring_monomer->getIdx());
    const auto attachment_vector =
        conformer.getAtomPos(first_chain_monomer->getIdx()) - ring_pos;

    const bool is_horizontal =
        std::abs(attachment_vector.x) >= std::abs(attachment_vector.y);
    if (is_horizontal) {
        const auto chain_direction =
            attachment_vector.x < 0 ? ChainDirection::RTL : ChainDirection::LTR;
        const auto x_offset = attachment_vector.x < 0 ? -MONOMER_BOND_LENGTH
                                                      : MONOMER_BOND_LENGTH;
        lay_out_chain(polymer, first_chain_monomer, placed_monomers_idcs,
                      ring_pos + RDGeom::Point3D(x_offset, 0, 0),
                      chain_direction);
        return;
    }

    // lay_out_chain generates horizontal coordinates. Generate to the right,
    // then rotate only the newly placed chain around its ring attachment point.
    const auto previously_placed = placed_monomers_idcs;
    lay_out_chain(polymer, first_chain_monomer, placed_monomers_idcs,
                  ring_pos + RDGeom::Point3D(MONOMER_BOND_LENGTH, 0, 0),
                  ChainDirection::LTR);
    const auto angle = attachment_vector.y < 0 ? -PI / 2 : PI / 2;
    const auto sin_angle = std::sin(angle);
    const auto cos_angle = std::cos(angle);
    for (const auto* monomer : polymer.atoms()) {
        const auto original_idx = monomer->getProp<int>(ORIGINAL_INDEX);
        if (previously_placed.contains(original_idx) ||
            !placed_monomers_idcs.contains(original_idx)) {
            continue;
        }
        const auto monomer_idx = monomer->getIdx();
        const auto rotated_pos = rotate_point(conformer.getAtomPos(monomer_idx),
                                              sin_angle, cos_angle, ring_pos);
        conformer.setAtomPos(monomer_idx, rotated_pos);
    }
}
 ...
 * be assigned to all monomers, but only the ones in rings will be marked as
 * placed in `placed_monomers_idcs`, so that all chain coordinates can be
 * overwritten later.
 */
static void
                                ...
                                std::unordered_set<int>& placed_monomers_idcs)
{
    ...
    // Finalize only ring coordinates. Keeping the first non-ring neighbor would
    // preserve RDKit's diagonal attachment geometry and force the rest of the
    // chain to continue horizontally from that offset.
                       ...
                       std::unordered_set<int>& placed_monomers_idcs)
{
    ...
    for (auto ring_atoms : polymer.getRingInfo()->atomRings()) {
        for (auto monomer_idx : ring_atoms) {
            auto monomer = polymer.getAtomWithIdx(monomer_idx);
            // Starting from each atom bound to a ring, lay out chains in the
            // closest horizontal or vertical direction away from the ring.
            for (auto neighbor : polymer.atomNeighbors(monomer)) {
                ...
                lay_out_chain_from_ring(polymer, monomer, neighbor,
                                        placed_monomers_idcs);
            }
        }
    }
