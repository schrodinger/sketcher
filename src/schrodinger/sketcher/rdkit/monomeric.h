...
/**
 * @return the Watson-Crick complement symbol for the given nucleic acid base
 * residue name, or std::nullopt if the symbol has no standard complement.
 * G/C pairing is shared between DNA and RNA; the purine A pairs with T in DNA
 * and U in RNA, while both pyrimidines T and U pair back to A.
 * @param base_symbol the residue name of the base (e.g. "A", "G")
 * @param is_dna whether the complement being generated is DNA, which selects
 * T (DNA) vs U (RNA) as the complement of A
 */
