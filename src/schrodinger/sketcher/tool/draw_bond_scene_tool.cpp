...
{
    m_mol_model->addAtom(Element::C, pos, m_bond_type, m_bond_dir, bound_to);
}

void DrawBondSceneTool::addTwoBoundAtoms(const RDGeom::Point3D& pos1,
                                         const RDGeom::Point3D& pos2)
{
    m_mol_model->addAtomChain(Element::C, {pos1, pos2}, m_bond_type,
                              m_bond_dir);
}
...
DrawBondQuerySceneTool::DrawBondQuerySceneTool(BondTool bond_tool, Scene* scene,
    ...
    AbstractDrawBondSceneTool(scene, mol_model)
{
    // map of bond tool to {whether the bond is flippable (i.e. does the bond
    // change meaningfully if we swap the start and end atoms)}
    // TODO: add query data here
    std::unordered_map<BondTool, bool> query_type_map = {
        {BondTool::SINGLE_EITHER, true},
        {BondTool::DOUBLE_EITHER, false},
        {BondTool::SINGLE_OR_DOUBLE, false},
        {BondTool::SINGLE_OR_AROMATIC, false},
        {BondTool::DOUBLE_OR_AROMATIC, false},
        {BondTool::ANY, false}};
    auto query_info = query_type_map.find(bond_tool);
    if (query_info == query_type_map.end()) {
        m_flippable = query_info->second;
    } else {
        throw std::runtime_error(
