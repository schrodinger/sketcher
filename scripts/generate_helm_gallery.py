...
def get_html_header(num_entries: int) -> str:
    """Generate the HTML header with entry count."""
    ...
    <div class="max-w-7xl mx-auto">
        <header class="mb-12 border-b border-slate-200 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
                ...
                <p class="text-slate-500 mt-2 text-lg">Visualizing {num_entries} monomeric entries.</p>
            </div>
...
def generate_card_html(entry: HelmEntry, svg_content: str) -> str:
    """Generate HTML for a single gallery card."""
...
def generate_svg(helm_string: str, helm_to_svg_path: str) -> str | None:
...
def create_webpage(entries: List[HelmEntry], output_file: str, helm_to_svg_path: str):
    ...
    for entry in entries:
        ...
        successful += 1
        html_cards.append(generate_card_html(entry, svg_content))

    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(get_html_header(len(entries)) + "".join(html_cards) + HTML_FOOTER)
